    ...
    async.waterfall([
        ...
        function waterfall6(destinationBucket, dataLocations,
            ...
            if (serverSideEncryption) {
                ...
                for (let i = 0; i < storedPartsAsObjects.length; i++) {
                    const part = storedPartsAsObjects[i];
                    if (part.sseAlgorithm !== pseudoCipherBundle.algorithm) {
                        next(errors.InvalidPart);
                        return;
                    }
                    if(part.sseMasterKeyId !== pseudoCipherBundle.masterKeyId) {
                        next(errors.InvalidPart);
                        return;
                    }
                }
