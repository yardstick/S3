...
describe('Multipart Upload API', () => {
    ...
    it('should upload a part', done => {
        ...
        (err, json) => {
            ...
            const bufferBody = Buffer.from(postBody, 'binary');
            md5Hash.update(bufferBody, 'binary');
    ...
    '(and the stored ETag in metadata should be hex)', done => {
        ...
        (err, json) => {
            ...
            const calculatedHash =
                md5Hash.update(bufferBody, 'binary').digest('hex');
            const partRequest = new DummyRequest({
                bucketName,
                namespace,
                objectKey,
    ...
    it('should upload two parts', done => {
        ...
        (err, json) => {
            ...
            objectPutPart(authInfo, partRequest1, log, () => {
                const postBody2 = Buffer.from('I am a second part');
                const md5Hash2 = crypto.createHash('md5');
                const bufferBody2 = Buffer.from(postBody2, 'binary');
                md5Hash2.update(bufferBody2, 'binary');
    ...
    it('should complete a multipart upload', done => {
        ...
        (err, json) => {
            ...
            const md5Hash = crypto.createHash('md5').update(partBody, 'binary');
            const calculatedHash = md5Hash.digest('hex');
            const partRequest = new DummyRequest({
                bucketName,
                namespace,
    ...
    it('should not leave orphans in data when overwriting an object with a MPU',
        ...
        async.waterfall([
            ...
            (json, next) => {
                ...
                const md5Hash = crypto.createHash('md5')
                    .update(overwritePartBody, 'binary');
                const calculatedHash = md5Hash.digest('hex');
                const partRequest = new DummyRequest({
                    bucketName,
                    namespace,
